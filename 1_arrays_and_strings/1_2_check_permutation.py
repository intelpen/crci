...
def count_chars_with_list(in_str):
    counts = [0]*256
    if in_str is None:
        return counts
    for character in in_str:
...
def check_permutation_with_list(first_str, second_str):
    return count_chars_with_list(first_str) == count_chars_with_list(second_str)
...
class TestCheckPermutation(unittest.TestCase):
    ...
    def test_check_permutations_with_list(self):
        self.run_checks(check_permutation_with_list)